...
class Cache: # manages in-memory storage and reduces unnecessary external API network queries.
    def __init__(self):
    ...
    def load(self, api_client): # loads the reference data from the API client and stores it in memory.
    ...
    def save_to_file(self, filename): # saves the reference cache data to a local JSON file.
        data_to_save = {
...
class Clienhandler: # manages an isolated connected socket user client's network life cycles.
    ...
    def start(self): # spawns a background listener daemon thread.
        thread = threading.Thread(target=self.handle, daemon=True)
        thread.start()

    def handle(self): # monitors socket traffic and validates identity handshakes.
        try:
           msg = self._recv()
        ...
        except (ConnectionResetError, BrokenPipeError, OSError) as e: # capture unpredictable connection crashes
            self._log(f"Connection error : {e}")

        finally:
            # always make sure that connection lifecycle teardowns are always clean.
            self.conn.close()
            self._log("Connection closed")

    def _process_request(self, req_type, params): # switchboard for internal request routing.
    ...
    def _send(self, obj): # converts Python data dictionaries to JSON wire strings with 4-byte framing.
        
        data = json.dumps(obj).encode()
        length = len(data).to_bytes(4, "big")
        self.conn.sendall(length + data)

    def _recv(self): # reads the predictable 4-byte message block prefix header.
...
class Server: # main controller engine managing port assignments, cache bindings, and multi-threaded socket listener infrastructure loops.
    ...
    def start(self): # main ignition framework method.
    ...
    def _accept_loop(self): # loops infinitely on the listening socket and maps clients to handler threads.
        try:
            while True:
